    ...
    moduleServer(id, function(input, output, session) {
        ...
        observe({
                if (!is.null(input$data_variables_file)) {
                    data_variables <- fread(file = input$data_variables_file$datapath, sep = "auto", dec = ".", header = T, stringsAsFactors = TRUE)
        ...
        output$normalized_count_data_table <- DT::renderDataTable(DT::datatable(
            {
                # browser()
                if (is.null(current_data$bn_df_taxas)) {
                    return(NULL)
                }
                if (input$filter_taxa & input$before_after_filter == "Before") {
                    if (current_data$apply_count_filters != input$apply_count_filters) {
                        ## just pressed the btn
                        print(input$apply_count_filters)
                        current_data$apply_count_filters <- input$apply_count_filters
