    ...
    div(
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          div(style = "padding: 50 px 0 px; width: 100 px", HTML('<h5 style="position: relative;">Run The build here, then use filter options from Build->Load Netowrk->Filter,after loading the result</h5>'))
        ),
        conditionalPanel(
          ...
          conditionalPanel(
            ...
            div(style = "padding: 50 px 0 px; width: 100 px", numericInput(ns("filter_thrT"), "Select global filter threshold", value = 5, min = 0, max = 100, step = 0.5)),
            # conditionalPanel(
            #   condition = "input.before_after_filter == 'After'", ns = ns,
          ...
          conditionalPanel(
            ...
            numericInput(ns("filter_countsG"), "Specify a minimum number of counts to apply this filter", value = 10, min = 0, step = 1),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    current_bg_process <- eventReactive(input$start_net, {
      ...
      if (input$filter_taxa == 1) {
        if (input$before_after_filter == " After") {
          taxa_count_filters <- NULL
        } else {
          ...
          # )
          taxa_count_filters <- app_data$taxa_count_filters
        }
      }
    ...
    output$group_filter_thrG_input <- renderUI({
      if (is.null(input$filter_variable)) {
        ...
        data_type <- class(current_data$orginal_bn_df_variables[[input$filter_variable]])
        if (data_type == "factor") {
          level_lists <- levels(current_data$orginal_bn_df_variables[[input$filter_variable]])
          tags <- tagList(c())
    ...
    observe({
      ...
      if (!is.null(input$data_taxas_file)) {
        ...
        tryCatch(
          {
            data_taxas <- fread(
              file = input$data_taxas_file$datapath,
              sep = "auto", dec = ".", header = T, stringsAsFactors = TRUE
            )
    ...
    validate_filter_options <- function() {
      ##### Test Filters
      
      validate(
        need(current_data$orginal_bn_df_taxas, "Please load Data first")
      )
      taxa_count_filters <- NULL
      if (input$before_after_filter == "Before") {
        ...
        if (input$filter_option == "Group") {
          ...
          if (data_type != "factor") {
            validate(
                need(FALSE, paste("The selected Variable can not be used, It is not factor : ", input$filter_variable)),
              )
          }
          level_lists <- levels(current_data$orginal_bn_df_variables[[input$filter_variable]])
          for (i in seq_len(length(level_lists))) {
      ...
      app_data$taxa_count_filters <- taxa_count_filters
    }
