...
#  })
  ...
  observeEvent(input$network_plot,{if ( is.null(input$network_plot)) return(NULL)
    ...
    observeEvent( eventExpr = input$button_plot , ignoreNULL = FALSE, {  
      ...
      #Undo()
      updateCheckboxGroupInput( inputId = "Filter_Tab_N", 
                                choices = list("Emphasize" = "Emph", "Show/Hide" = "S/H"), 
                                selected = NULL)
      updateCheckboxGroupInput( inputId = "Filter_Tab_E", 
                                choices = list("Emphasize" = "Emph", "Show/Hide" = "S/H"), 
                                selected = NULL) 
      updateCheckboxGroupInput( inputId = "Filter_Menu", 
                                choices = list("Emphasize" = "Emph", "Show/Hide" = "S/H"), 
                                selected = NULL)
      updateCheckboxGroupInput(inputId = "Filter_Menu_By",  
    ...
    output$network_proxy <- renderVisNetwork(
      ...
      p_button()  %>% 
        ...
        visNodes(color = list(background = DT$color_background,
                              border = DT$color_border,
                              highlight = list(background = DT$color_highlight,
                                               border = DT$color_border
                              )),
                 physics = DT$bounce, 
                 shape=DT$shape)  %>%
        visEdges(scaling = list( min = DT$scaling_min, 
                                 max = DT$scaling_max,
                                 label = list(enabled = DT$scaling_label_enabled,
    ...
    OrderNodes <- function(){
      ...
      if( isFALSE(input$Reorder_N) | is.null(input$network_proxy_selectedNodes)){
        ...
        Sel_tab <- (merge ( x = data.frame(ord = (1:length(input$network_proxy_selectedNodes)),
                                           node = input$network_proxy_selectedNodes),
                            y = data.frame(ind = Ind_nodes, 
    ...
    observeEvent( eventExpr = input$Tab_nodes_cell_edit, { 
      ...
      if ( colnames(Ord_nod_tab)[Nodes_edit$col] == "label"){ 
        ...
        visNetworkProxy("network_proxy") %>%
          visUpdateNodes(nodes= data.frame(id = ID,
    ...
    output$Value_decimals <- renderUI({
      if( ! is.null(Tab_inputs$edges$value)){ 
        ...
        sliderInput( inputId = "Value_decimals", label = "Significant digits", 
    ...
    OrderEdges <- function(){
      if( isFALSE(input$Reorder_E) | is.null(input$network_proxy_selectedEdges)){ 
        ...
        Sel_tab <- (merge ( x = data.frame(ord = (1:length(input$network_proxy_selectedEdges)),
                                           edge =input$network_proxy_selectedEdges),
                            y = data.frame(ind = Ind_edges, 
    ...
    observeEvent( eventExpr = input$Value_decimals, ignoreNULL = TRUE, {  
      ...
      if( isTRUE(input$E_label)){ 
        ...
        visNetworkProxy("network_proxy") %>%
          visUpdateEdges(edges = data.frame(id = edges_info$id,
    ...
    observe({
      if (isTRUE(input$N_show_shadow)){
        if(! is.null(nodes)){ 
          #S_nodes = Select_nodes() 
          visNetworkProxy("network_proxy") %>%
            visUpdateNodes(nodes= data.frame(id = nodes$id, 
                                             shadow = list(enabled = input$N_show_shadow,
                                                           size = input$N_size,
                                                           x = input$N_shadow_x,
                                                           y= input$N_shadow_y)) ) 
        } else{
    ...
    observeEvent( eventExpr = input$E_direction, {
      if(! is.null(edges)){ 
        S_edges = Select_edges( )
        visNetworkProxy("network_proxy") %>%
          visUpdateEdges(edges= data.frame(id = S_edges, 
    ...
    observe({
      if(! is.null(edges)){ 
        if (isTRUE(input$E_show_shadow)){
          S_edges = Select_edges() 
          visNetworkProxy("network_proxy") %>%
            visUpdateEdges(edges= data.frame(id = edges$id, 
                                             shadow = list(enabled = input$E_show_shadow, 
                                                           x = input$E_shadow_x,
                                                           y= input$E_shadow_y)) )
        } else{
          visNetworkProxy("network_proxy") %>%
    ...
    Emphasize_N <<- function(S_nodes){  
      ...
      if (length(Hide_nodes)>0){ 
        visNetworkProxy("network_proxy") %>%
          visUpdateNodes(nodes= data.frame(id = Hide_nodes, 
                                           color = H_color , label = "")) %>% 
          visUpdateEdges(edges = data.frame(id = Hide_edges, 
                                            color = H_color , label = " "))
      }  
    }
    ...
    Undo_N <<- function(){
      ...
      if(isTRUE(input$N_label)){
        visNetworkProxy("network_proxy") %>%
          visUpdateNodes(nodes= data.frame(id = nodes_info$id, 
                                           hidden = FALSE,
                                           # Color
                                           nodes_info[, grep("color.",
                                                             colnames(nodes_info))], 
                                           # Etiqueta
                                           label = nodes_info$label)) 
      }else{
      ...
      if ( (! is.null(input$Filter_Menu) && length(input$Filter_Menu) == 1)){  
        if (! is.null(input$Filter_Menu_By) && length(input$Filter_Menu_By) == 1 ){ 
          if( input$Filter_Menu_By == "By_sel" || (! is.null(input$Filter_by) && input$Filter_by != "All" && input$Filter_by != "None")){ 
            sliderInput( inputId = "Filter_grade", label = "Grade", 
      ...
      if ( (! is.null(input$Filter_Menu) && length(input$Filter_Menu) == 1)){  
        if (! is.null(input$Filter_Menu_By) && length(input$Filter_Menu_By) == 1 ){  
          if  (input$Filter_Menu_By != "By_group" || (! is.null(input$Filter_by) && input$Filter_by != "All" && input$Filter_by != "None")){ 
            radioButtons( inputId = "Filter_direction", label = "Direction",
      ...
      if ( (! is.null(input$Filter_Menu) && length(input$Filter_Menu) == 1)){ 
        if (! is.null(input$Filter_Menu_By) && length(input$Filter_Menu_By) == 1 ){ 
          if  (input$Filter_Menu_By == "By_num" ){ 
            ...
            sliderInput( inputId = "Filter_number", label = "Minimum number of conecctions", 
    ...
    output$Remove_group <- renderUI(  
      if(! is.null(input$Filter_by) && ( ! is.null(input$Filter_Menu ) ) && ! is.null(input$Filter_Menu_By )){
        if(input$Filter_Menu_By == "By_group" && input$Filter_by != "None" && input$Filter_by != "All" ){
          ...
          actionButton( inputId = "remove_group", 
                        icon("remove"),  
    ...
    observeEvent( eventExpr = input$Capture,{ 
      shinyscreenshot::screenshot(selector ='#network_proxy', 
    ...
    output$network_save <- renderVisNetwork({ 
      ...
      if (input$Out_type == "HTML"){  
        ...
        visNetwork(nodes_out, edges_out)  %>%    
          ...
          visExport(type = input$Out_type, name = input$Out_name, float = "right",
                    style = DT$st) 
      }
    ...
    observe({
      visNetworkProxy("network_save") %>%
        visSetTitle(main = list(text = input$Title_main ),
    ...
    output$HTML_Button <- renderUI({
      if(input$Out_type == "HTML"){
        absolutePanel(actionButton(inputId = "Save_HTML",
    ...
    output$Output_SVG <- downloadHandler(filename = "salida.svg", content= function(filename){
      ...
      nodes_out$y <- sapply(nodes_out$y, function(y){0-y} )
